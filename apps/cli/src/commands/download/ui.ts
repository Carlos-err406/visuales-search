...
import { setDownloadRenderer, type DownloadBar, type ProgressBar } from "@visuales/core/download/ui";
...
export function resetDownloadBar(bar: DownloadBar) {
...
setDownloadRenderer({
  progressBars: {
    log: (message) => progressBars.log(message),
    remove: (bar) => progressBars.remove(bar as cliProgress.SingleBar),
    stop: () => progressBars.stop(),
  },
  createDownloadBar,
