...
- An available update opens a compact notice across Search, Downloads, Settings, and About. Installed version, last successful check, routine status, and manual checks live in the right-aligned About tab; there is no header up-arrow.
- Later hides an available-update notice until the next scheduled check or app launch. The checked release remains downloadable in About. Download progress, errors from a download attempt, and restart readiness remain visible across views.
