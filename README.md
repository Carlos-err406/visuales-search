...
Missing or inaccessible folders (403/404/410) count as skipped for the current scan, without pausing unrelated indexing. A 404/410 branch also defers its descendants while preserving cached results; Retry indexing checks the parent again and restores deferred work if it becomes available. Other folder failures receive at most three attempts per scan, including the first request, with the oldest waiting retry handled first. Exhausted failures remain visible and are deferred until the next daily scan or an explicit retry; they are not treated as deleted folders. Attempt budgets survive scanner restarts. Network failures and server throttling still use backoff, and server Retry-After deadlines remain in force even after Retry or Refresh. Background listings remain serial and paced, with slow responses counting toward the interval instead of adding an extra delay afterward.
...
Completion notifications offer **Show in Finder**, **Show in File Explorer**, or **Open containing folder**, depending on the OS. The action opens that transfer's saved output directory, even after changing download defaults. Missing folders show a warning. Actions work while the originating app session is running and depend on OS notification support; bare macOS dev executables send notification-only previews. See [notification behavior and platform checks](docs/desktop-notifications.md).
...
Preferences live in `~/.visuales-cli-cache/desktop-settings.json` and survive cache clearing. Each new desktop task captures its defaults at creation; resumed tasks use their stored options. An explicitly chosen download destination overrides the saved folder. CLI defaults and flags are unchanged.

Saving a changed **Concurrent files** value also updates this app instance's running and queued transfers, including selective file retries. Increasing from three to five immediately fills up to two additional file slots. Decreasing from five to three leaves all five files running: the first two completions do not start replacements, then the scheduler maintains the lower limit. The applied limit is stored with each task for resume. Other app instances and CLI workers are untouched, as are connections per file, retries, ignore rules, destinations, and queue order. A worker that cannot acknowledge the change produces a save error; Save again retries delivery without restarting its transfer.
