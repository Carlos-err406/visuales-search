...
async function loadCacheIndex(): Promise<CacheIndex> {
  ...
  try {
    ...
    logger.log(colors.yellow("⚠️  Cache index corrupted, creating new one"));
    const defaultIndex: CacheIndex = {
      version: "1.0.0",
      entries: [],
    };
...
async function saveCacheIndex(index: CacheIndex): Promise<void> {
  ...
  try {
    ...
    logger.log(colors.yellow("⚠️  Failed to save cache index"));
  }
}
...
export async function getCachedHtml(): Promise<string | null> {
  ...
  try {
    ...
    logger.log(colors.gray(`📦 Using cached data (${ageString})`));
    return data.html;
  } catch {
    logger.log(colors.yellow("⚠️  Cache file corrupted, will fetch fresh data"));
    return null;
  }
}
...
export async function setCachedHtml(html: string): Promise<void> {
  ...
  try {
    ...
    logger.log(colors.green("💾 Cached HTML"));
  } catch {
    logger.log(colors.yellow("⚠️  Failed to cache HTML"));
  }
}
...
function loadSearchAliasCache(): SearchAliasCache {
  ...
  try {
    ...
    logger.log(colors.yellow("⚠️  Search aliases cache corrupted, creating new one"));
    return {
      version: "1.0.0",
      updated: Date.now(),
      entries: {},
...
export async function clearStaleDownloadCache(maxAgeMs: number = 7 * 24 * 60 * 60 * 1000): Promise<void> {
  ...
  try {
    ...
    if (deletedCount > 0) {
      logger.log(colors.gray(`🗑️  Cleaned ${deletedCount} stale download cache files`));
