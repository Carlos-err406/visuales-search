...
- **Background & Queued Tasks (`--detach, -d` / `--queue, -q`)**: `-d` runs a download in a detached background process tracked in `visuales tasks`. `-q` marks the task `queued` so it parks until no other task is `running`, then starts — a serial FIFO queue (earliest-enqueued first). The two compose: `visuales download <id> -qd` queues a download in the background behind whatever is currently running. The same flags are available on `visuales tasks resume`. The gate is poll-based (no daemon) and treats a running task whose process has died as finished, so a completed _or_ interrupted download frees the next slot.
