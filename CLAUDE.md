...
- **Background & Queued Tasks (`--detach, -d` / `--queue, -q`)**: `-d` runs a download in a detached background process tracked in `visuales tasks`. `-q` parks it until no other task is running. The serial queue defaults to FIFO; `visuales tasks queue`, `tasks move <id> <position>`, and `tasks next <id>` inspect or change its shared persisted order. Desktop uses the same core operations. Reordering and claiming a queue slot are atomic under the task-store lock. Start next never interrupts running work. The flags compose (`-qd`) and are available on `visuales tasks resume`. The gate is poll-based (no daemon); a completed or interrupted download frees the next slot.
- **Integrity Verification**: Every finished file is checked _before_ it leaves the `.visuales-parts/` sidecar directory. A matching current exact server size passes without an extra request; cached sizes are only estimates for verification. Otherwise a single-byte range probe confirms the total and missing bytes are repaired. HTTP 416 without a total is not proof of completion. If the server discloses no usable total, the file/task fails with an explanation and partial data stays resumable, never promoted or counted as completed. Resume can reuse unchanged local completions only with explicit verification under the current verification version plus matching size and modification time; older records require one verification pass.
